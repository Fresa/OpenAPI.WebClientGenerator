...
public class ExportFooEventsTests(FooApplicationFactory app) : FooTestSpecification, IClassFixture<FooApplicationFactory>
{
    ...
    {
        var response = await SendAsync<Foo.Foo.Foo1.Events0.Get.Response.OK200.ApplicationJsonl>();
    ...
    {
        var response = await SendAsync<Foo.Foo.Foo1.Events0.Get.Response.OK200.ApplicationXJsonlines>();
        await AssertContentAsync(response.Content);
    }
    ...
    {
        var response = await SendAsync<Foo.Foo.Foo1.Events0.Get.Response.OK200.ApplicationXNdjson>();
        await AssertContentAsync(response.Content);
    }
    ...
    {
        var response = await SendAsync<Foo.Foo.Foo1.Events0.Get.Response.OK200.ApplicationJsonSeq>();
        await AssertContentAsync(response.Content);
    }
    ...
    {
        var response = await SendAsync<Foo.Foo.Foo1.Events0.Get.Response.OK200.ApplicationGeoJsonSeq>();
        await AssertContentAsync(response.Content);
    }
    
    private async Task<T> SendAsync<T>()
        where T : Foo.Foo.Foo1.Events0.Get.Response.OK200, Foo.Foo.Foo1.Events0.Get.Response.IAcceptContent
    {
        using var httpClient = app.CreateClient();
        var client = new Foo.Foo(httpClient);
        var result = await client.Foo_(1).Events().GetAsync(
            accepts: Foo.Foo.Foo1.Events0.Get.Response.Accept.Content<T>(), 
